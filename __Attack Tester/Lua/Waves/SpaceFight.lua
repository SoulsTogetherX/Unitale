-- Try to keep moving, you mostly just need reflexes.
timer = 0
timer2 = 0
bullets = {}
...
Shoot.SetVar("R",0)
Shoot2 = CreateProjectile("Spaceship2_1",-150,100)
Shoot2.sprite.Scale(1.5,1.5)
Shoot2.sprite.SetAnimation({"Spaceship2_1", "Spaceship2_2", "Spaceship2_3"},1/20) 
Shoot2.ppcollision = true
Shoot2.SetVar("Type",2)
...
function Update()
	...
	if timer > 25 then
		...
		if timer%120 == 0 then
			if Player.x - Shoot2.x >= 0 then
				Shoot2.sprite.rotation = math.deg(math.atan2(Player.y - Shoot2.y, Player.x - Shoot2.x))
			else
				Shoot2.sprite.rotation = math.deg(math.atan2(Player.y - Shoot2.y, Player.x - Shoot2.x)) - 180
			end
			local distance = math.sqrt((Player.x - Shoot2.x)^2 + (Player.y - Shoot2.y)^2)
			local distance_x = ((Player.x - Shoot2.x) / distance)*5
			local distance_y = ((Player.y - Shoot2.y) / distance)*5
			Shoot2.SetVar("distance_x", distance_x)
			Shoot2.SetVar("distance_y", distance_y)
			Wait = true
		end
		if Wait == true and timer2 < 10 then
			timer2 = timer2 + 1
		elseif timer2 >= 10 then
			timer2 = 0
			Wait = false
		end
		Shoot2.sprite.alpha = 1
		Shoot2.sprite.Scale(1.5,1.5)
		if Shoot2.GetVar("distance_x") ~= nil and Shoot2.GetVar("distance_y") ~= nil then
			Shoot2.Move(Shoot2.GetVar("distance_x"),Shoot2.GetVar("distance_y"))
		end
		if math.sqrt((0 - Shoot2.x)^2 + (100 - Shoot2.y)^2) > 150 and Wait == false then
			Shoot2.SetVar("distance_x", 0)
			Shoot2.SetVar("distance_y", 0)
		end
		if timer%30 == 0 then
			for i=1,2 do
				local distance = math.sqrt((Player.x - Shoot.x)^2 + (Player.y - Shoot.y)^2)
				local distance_x = ((Player.x - Shoot.x) / distance)*2
				local distance_y = ((Player.y - Shoot.y) / distance)*2
				...
				local distance_x = ((Inv1.x - Shoot.x) / distance)*2
				local distance_y = ((Inv1.y - Shoot.y) / distance)*2
				local bullet = CreateProjectile("Pixel",Shoot.x + (15*(distance_x/math.abs(distance_x))),Shoot.y + (15*(distance_y/math.abs(distance_y))))
				bullet.SetVar("distance_x", distance_x)
				bullet.SetVar("distance_y", distance_y)
				bullet.SetVar("Type",1)
				...
				local distance_x = ((Inv2.x - Shoot.x) / distance)*2
				local distance_y = ((Inv2.y - Shoot.y) / distance)*2
				local bullet = CreateProjectile("Pixel",Shoot.x + (15*(distance_x/math.abs(distance_x))),Shoot.y + (15*(distance_y/math.abs(distance_y))))
				bullet.SetVar("distance_x", distance_x)
				bullet.SetVar("distance_y", distance_y)
				bullet.SetVar("Type",1)
				...
				local distance_x = ((Inv3.x - Shoot.x) / distance)*2
				local distance_y = ((Inv3.y - Shoot.y) / distance)*2
				local bullet = CreateProjectile("Pixel",Shoot.x + (15*(distance_x/math.abs(distance_x))),Shoot.y + (15*(distance_y/math.abs(distance_y))))
				bullet.SetVar("distance_x", distance_x)
				bullet.SetVar("distance_y", distance_y)
				bullet.SetVar("Type",1)
				...
				local distance_x = ((Inv4.x - Shoot.x) / distance)*2
				local distance_y = ((Inv4.y - Shoot.y) / distance)*2
				local bullet = CreateProjectile("Pixel",Shoot.x + (15*(distance_x/math.abs(distance_x))),Shoot.y + (15*(distance_y/math.abs(distance_y))))
				bullet.SetVar("distance_x", distance_x)
				bullet.SetVar("distance_y", distance_y)
				bullet.SetVar("Type",1)
